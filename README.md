...
| [`update_issue_status`](#update_issue_status) | Переводит задачу в другой статус |
...
Переводит задачу по workflow Трекера. `status` — имя целевого статуса (`В работе`), его ключ (`inProgress`) или id перехода. Текущий статус читается через [`get_issue`](#get_issue): поле `status`. Если задача уже в этом статусе, переход не выполняется. Для закрывающих переходов часто нужен ключ резолюции, например `resolution=fixed`. Если переход из текущего статуса недоступен, ответ перечисляет доступные.
